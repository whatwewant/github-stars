/**
* @Author: eason
* @Date:   2017-01-12T12:06:05+08:00
* @Email:  uniquecolesmith@gmail.com
* @Last modified by:   eason
* @Last modified time: 2017-01-12T12:22:56+08:00
* @License: MIT
...
function extractPageFromUrl(url) {
  if (url) return url.match(/page=(\d+)/)[1]; // origin：url.split('&page=')[1];
}
...
export default {
  ...
  reducers: {
    ...
    'sync/save' (state, action) {
      return {...state, data: action.payload, syncAllFinished: false};
    },
  ...
  effects: {
    ...
    *unstar({ payload: repo }, { select, put, call }) {
      ...
      if (res.status !== 204) {
        throw new Error('Error: status should be 204');
      }
      yield put({
        type: 'unstar/end',
      });
